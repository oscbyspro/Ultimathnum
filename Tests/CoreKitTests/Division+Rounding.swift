...
extension DivisionTests.Case {
    ...
    func ceil(_ expectation: Fallible<Quotient>) {
        ...
        test.same(Fallible(item, error: true ).ceil(), expectation.combine(true), "Fallible/ceil() [1]")
    }
    ...
    func floor(_ expectation: Fallible<Quotient>) {
        ...
        test.same(Fallible(item, error: true ).floor(), expectation.combine(true), "Fallible/floor() [1]")
    }
}

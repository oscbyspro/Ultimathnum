...
extension Namespace.TupleBinaryInteger {
    ...
    @inlinable package static func magnitude(of value: consuming X2) -> X2.Magnitude {
        ...
        if  value.high.isLessThanZero {
            var carry:   Bool
            (value.low,  carry) = Overflow.capture({ try (~value.low ).plus(0000000000001) }).components
            (value.high, carry) = Overflow.capture({ try (~value.high).plus(carry ? 1 : 0) }).components
        }

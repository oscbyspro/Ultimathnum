...
extension IntegerInvariants {
    ...
    public func multiplicationAboutMsb(_ id: SystemsIntegerID) where T: SystemsInteger {
        //=--------------------------------------=
        let msb = T.msb
        let not = T.msb.toggled()
        //=--------------------------------------=
        always: do {
            test.multiplication(msb, msb, Fallible(T2(low:  0 as M,    high:  T(raw:  M.msb >> 1)), error: true))
            test.multiplication(msb, not, Fallible(T2(low:  M .msb,    high:  T(raw:    msb >> 1) - T(Bit(!T.isSigned))), error: true))
            test.multiplication(not, msb, Fallible(T2(low:  M .msb,    high:  T(raw:    msb >> 1) - T(Bit(!T.isSigned))), error: true))
            test.multiplication(not, not, Fallible(T2(low:  1 as M,    high:  T(raw: ~M.msb >> 1)), error: true))
        };  if T.isSigned {
            test.multiplication(not,  ~4, Fallible(T2(low:  M.msb + 5, high: ~0000002), error: true))
            test.multiplication(not,  ~3, Fallible(T2(low:  000000004, high: ~0000001), error: true))
            test.multiplication(not,  ~2, Fallible(T2(low:  M.msb + 3, high: ~0000001), error: true))
            test.multiplication(not,  ~1, Fallible(T2(low:  000000002, high: ~0000000), error: true))
            test.multiplication(not,  ~0, Fallible(T2(low:  M.msb + 1, high: ~0000000)))
            test.multiplication(not,   0, Fallible(T2(low:  000000000, high:  0000000)))
            test.multiplication(not,   1, Fallible(T2(low: ~M.msb - 0, high:  0000000)))
            test.multiplication(not,   2, Fallible(T2(low: ~000000001, high:  0000000), error: true))
            test.multiplication(not,   3, Fallible(T2(low: ~M.msb - 2, high:  0000001), error: true))
            test.multiplication(not,   4, Fallible(T2(low: ~000000003, high:  0000001), error: true))
            ...
            test.multiplication(not,  ~4, Fallible(T2(low:  M.msb + 5, high:  not - 3), error: true))
            test.multiplication(not,  ~3, Fallible(T2(low:  000000004, high:  not - 2), error: true))
            test.multiplication(not,  ~2, Fallible(T2(low:  M.msb + 3, high:  not - 2), error: true))
            test.multiplication(not,  ~1, Fallible(T2(low:  000000002, high:  not - 1), error: true))
            test.multiplication(not,  ~0, Fallible(T2(low:  M.msb + 1, high:  not - 1), error: true))
            test.multiplication(not,   0, Fallible(T2(low:  000000000, high:  0000000)))
            test.multiplication(not,   1, Fallible(T2(low: ~M.msb - 0, high:  0000000)))
            test.multiplication(not,   2, Fallible(T2(low: ~000000001, high:  0000000)))
            test.multiplication(not,   3, Fallible(T2(low: ~M.msb - 2, high:  0000001), error: true))
            test.multiplication(not,   4, Fallible(T2(low: ~000000003, high:  0000001), error: true))

            test.multiplication(msb,  ~4, Fallible(T2(low:  M.msb,     high:  not - 2), error: true))
            test.multiplication(msb,  ~3, Fallible(T2(low:  000000000, high:  not - 1), error: true))
            test.multiplication(msb,  ~2, Fallible(T2(low:  M.msb,     high:  not - 1), error: true))
            test.multiplication(msb,  ~1, Fallible(T2(low:  000000000, high:  not - 0), error: true))
            test.multiplication(msb,  ~0, Fallible(T2(low:  M.msb,     high:  not - 0), error: true))
            test.multiplication(msb,   0, Fallible(T2(low:  000000000, high:  0000000)))
            test.multiplication(msb,   1, Fallible(T2(low:  M.msb,     high:  0000000)))
